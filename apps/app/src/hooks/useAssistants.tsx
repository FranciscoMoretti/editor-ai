...
export function useAssistants() {
  ...
  const { mutateAsync: deleteAssistantMutationAsync } =
    api.assistants.delete.useMutation();
  const { mutateAsync: createAssistantMutationAsync } =
    api.assistants.create.useMutation();
  const { mutateAsync: updateAssistantMutationAsync } =
    api.assistants.update.useMutation();
  const utils = api.useUtils();

  const getAssistants = async (userId: string): Promise<void> => {
    setIsLoadingAllAssistants(true);
    try {
      const userAssistants = await utils.assistants.findMany
        .fetch({
          // metadata: {
          //   user_id: userId,
          // },
        })
        .then((assistants) => assistants.map(responseAssistantToAssistant));
  ...
  const deleteAssistant = async (assistantId: string): Promise<boolean> => {
    ...
    try {
      await deleteAssistantMutationAsync({
        where: {
  ...
  const createCustomAssistant = async (
    ...
    try {
      ...
      const createdAssistantResponse = await createAssistantMutationAsync({
        data: {
          graph_id: "agent",
          name,
          metadata: {
      ...
      const createdAssistant = responseAssistantToAssistant(
  ...
  const editCustomAssistant = async (
    ...
    try {
      ...
      const response = await updateAssistantMutationAsync({
        assistant_id: assistantId,
        name,
        graph_id: "agent",
        metadata: {
      ...
      }).then(responseAssistantToAssistant);
  ...
  const getOrCreateAssistant = async (userId: string) => {
    ...
    try {
      userAssistants = await utils.assistants.findMany
        .fetch({
          where: {
            graph_id: "agent",
            // metadata: {
...
function responseAssistantToAssistant(assistant: any): Assistant {
  return {
    ...assistant,
    // TODO: Fill with assistant config from the endpoint
    updated_at: assistant.updated_at.toISOString(),
    created_at: assistant.created_at.toISOString(),
    config: { configurable: {} },
    metadata: assistant.metadata as Assistant["metadata"],
  };
}
